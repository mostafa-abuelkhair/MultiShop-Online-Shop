...
let categories_menu = new getAndRender("/api/categories/");

categories_menu.render = (d) => {
  let htm = ``;
  for (let product of d) {
    htm += `
          <a data-id="${product._id}" onclick="getCities(${product._id})" href="shop.html?cat_id=${product._id}" class="nav-item nav-link">${product.name}</a>
          `;
  }
